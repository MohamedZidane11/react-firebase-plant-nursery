...
const NurseryForm = () => {
  ...
  const [mapsUrlValidating, setMapsUrlValidating] = useState(false);
  const [mapsUrlError, setMapsUrlError] = useState('');
  const [mapsUrlSuccess, setMapsUrlSuccess] = useState(false);
  ...
  const validateGoogleMapsUrl = async (url) => {
    if (!url || !url.trim()) {
      setMapsUrlError('');
      setMapsUrlSuccess(false);
      return false;
    }
  
    setMapsUrlValidating(true);
    setMapsUrlError('');
    setMapsUrlSuccess(false);
  
    try {
      // إنشاء location string من البيانات الموجودة
      const locationString = formData.location || 
        `${formData.district}، ${formData.city}، ${formData.region}`.replace(/،\s*،/g, '،').trim();
  
      const response = await fetch(`${API_BASE}/api/validate-maps-url`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ 
          url: url.trim(),
          location: locationString  // ← إضافة location كـ fallback
        }),
      });
      ...
      if (data.valid) {
        setMapsUrlSuccess(true);
        setFormData(prev => ({
          ...prev,
          googleMapsEmbedUrl: data.embedUrl
        }));
        
        // إظهار رسالة إعلامية إذا تم استخدام fallback
        if (data.fallback) {
          console.info('ℹ️ تم استخدام نص الموقع لإنشاء الخريطة');
        }
        
        return true;
      } else {
        setMapsUrlError(data.error || 'رابط خرائط Google غير صالح');
        return false;
      }
    } catch (err) {
      setMapsUrlError('فشل التحقق من رابط الخريطة');
      console.error('Maps URL validation error:', err);
      return false;
    } finally {
      setMapsUrlValidating(false);
    }
  };

  const generateMapFromLocation = async () => {
    if (!formData.region || !formData.city) {
      alert('يرجى إدخال المنطقة والمدينة أولاً');
      return;
    }
    ...
    try {
      const locationString = `${formData.district}، ${formData.city}، ${formData.region}`
        .replace(/،\s*،/g, '،')
        .trim();
      ...
      if (data.valid) {
        setFormData(prev => ({
          ...prev,
          googleMapsEmbedUrl: data.embedUrl,
          googleMapsUrl: '' // مسح الرابط القديم
        }));
        setMapsUrlSuccess(true);
        setMapsUrlError('');
        alert('✅ تم إنشاء الخريطة من الموقع بنجاح!');
      }
    } catch (err) {
      console.error('Error generating map:', err);
      alert('فشل إنشاء الخريطة');
    } finally {
      setMapsUrlValidating(false);
    }
  };
      ...
      try {
        ...
        if (id) {
          ...
          if (nurseryDoc.exists()) {
            ...
            // Set success state if Google Maps URL exists
  ...
  const handleChange = (e) => {
    ...
    // Validate Google Maps URL on change
  ...
  const deleteVideo = async (index) => {
    ...
    await deleteImageFromStorage(videoUrl);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.googleMapsUrl && formData.googleMapsUrl.trim()) {
      if (!mapsUrlSuccess) {
        const isValid = await validateGoogleMapsUrl(formData.googleMapsUrl);
        if (!isValid) {
          alert('رابط خرائط Google غير صالح. يرجى استخدام رابط صحيح من تطبيق خرائط Google.');
          return;
        }
      }
    }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Google Maps Link with validation */}
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  <input
                    ...
                    className={`w-full px-4 py-2 border rounded focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                      mapsUrlError ? 'border-red-500' : 
                      mapsUrlSuccess ? 'border-green-500' : 
                      'border-gray-300'
                    }`}
                ...
                <button
                  type="button"
                  onClick={generateMapFromLocation}
                  disabled={mapsUrlValidating || !formData.region || !formData.city}
                  className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed whitespace-nowrap text-sm"
                  title="إنشاء خريطة من الموقع"
                >
                  إنشاء تلقائي
                </button>
              </div>
              ...
              <p className="text-xs text-gray-500">
                يمكنك لصق رابط من خرائط Google أو الضغط على "إنشاء تلقائي" لإنشاء خريطة من الموقع
              </p>
              
              {formData.googleMapsEmbedUrl && (
                <div className="mt-3 border rounded-lg overflow-hidden bg-gray-50">
                  <p className="text-xs text-gray-600 px-3 py-2 border-b">معاينة الخريطة:</p>
                  <iframe
            ...
            <div className="flex gap-4 pt-4">
              <button
                type="submit"
                disabled={loading || !!nameError || (formData.googleMapsUrl && mapsUrlValidating)}
